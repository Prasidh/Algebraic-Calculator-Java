    ...
    makeVariableLists(String expr, ArrayList<Variable> vars, ArrayList<Array> arrays) {
    	...
    	for(int i = 0;i<len;i++) {
    		...
    		if(Character.isLetter(expr.charAt(i))) {
    			String str = "";
    			int lastIndex = 0;
    			for(int j = i;Character.isLetter(expr.charAt(j));j++) {
    				str+= expr.charAt(j);
    				lastIndex = j;
    				if(j+1 == expr.length()) {
    					break;
    			...
    			if(lastIndex+1 != expr.length() && expr.charAt(lastIndex+1)=='[') { //ARRAY
    				for(int k = 0;k<arrays.size();k++) {
    					if(arrays.get(k).name.equals(str)) {
    						exists = true;
    					}
    				}
    				if(!exists) {
    					Array newArray = new Array(str);
        				arrays.add(newArray);
    				}
    			}else { //VARIABLE
    				for(int k = 0;k<vars.size();k++) {
    					if(vars.get(k).name.equals(str)) {
    						exists = true;
    					}
    				}
    				if(!exists) {
